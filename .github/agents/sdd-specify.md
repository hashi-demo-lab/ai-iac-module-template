...
2. **Draft**: Populate the `spec.md` created by the script, following `tf-spec-writing` skill patterns
3. **Validate**: Confirm all mandatory sections present, requirements testable, success criteria measurable, no implementation leakage
...
- **Module Interface Requirements**: Key input variables the module must accept, key outputs it must expose, and optional feature flags for conditional behavior
